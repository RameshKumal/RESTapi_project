...
class Feed extends Component {
  ...
  componentDidMount() {
    fetch('http://localhost:4000/feed/status', {
      headers: {
    ...
    //socket client side connection establishment.
    const socket = openSocket('http://localhost:4000')
    socket.on('posts', data => {
      if (data.action === 'create') {
        this.addPost(data.post);
      } else if (data.action === 'update') {
        this.postUpdated(data.post);
  ...
  //this will update everywhere if the new post is created.
  addPost = post => {
  ...
  statusUpdateHandler = event => {
    ...
    fetch(`http://localhost:4000/feed/update-status`, {
      method: "PATCH",
      headers: {
        "Authorization": `Bearer ${this.props.token}`,
